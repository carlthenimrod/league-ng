...
import { Slot } from '@app/models/place';

@Component({
  selector: 'app-admin-time-adder',
  templateUrl: './admin-time-adder.component.html',
  styleUrls: ['./admin-time-adder.component.scss']
})
export class AdminTimeAdderComponent implements OnInit {
  @Output() cancel: EventEmitter<boolean> = new EventEmitter();
  @Output() datesAdded: EventEmitter<Slot[]> = new EventEmitter();
  timeForm: FormGroup;
  triggerText: string;
  ...
  onSubmit() {
    ...
    if (occurences === 'single') {
      ...
      this.datesAdded.emit(this.createDate(date, time));
    } else {
      const {dates, time, days} = this.timeForm.value;
      this.datesAdded.emit(this.createDates(dates, time, days));
    }
  }
